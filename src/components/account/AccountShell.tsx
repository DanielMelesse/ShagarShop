...
const baseLinks = [
  { href: ACCOUNT_HOME, label: "Overview" },
  { href: ACCOUNT_PROFILE, label: "Profile & security" },
  { href: ACCOUNT_ORDERS, label: "Orders" },
] as const;
...
export function AccountNav() {
  ...
  const links = isSeller
    ? [
        ...baseLinks,
        { href: ACCOUNT_SHOP, label: "Shop settings" },
        { href: SELLER_HOME, label: "Seller dashboard" },
      ]
    : baseLinks;
...
export function AccountShell({ title, description, children }: AccountShellProps) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      <header>
        <p className="text-sm font-medium text-brand-700">My account</p>
